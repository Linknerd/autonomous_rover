...
void movement_setup() {
  ...
  // This utilizes the ATmega4809's capability to interrupt on any digital pin
  // natively.
...
// --- Right Encoder (4X) ---
// Note: Intentionally inverted counting direction to match previous logic
// natively
void ISR_encoderR_A() {
...
static short feedforward(double desired) {
  if (fabs(desired) < 1e-4)
    return 0;
  double mag =
      (fabs(desired) / MAX_WHEEL_SPEED) * (255.0 - DEADZONE) + DEADZONE;
...
void PID_update() {
  ...
  // [DEBUG] Old logic cleared the interrupts. We now sample and compute
  // difference
  // // noInterrupts();
